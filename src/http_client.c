...
int send_https_request(HttpRequest *request, char *response, int response_size) {
    ...
    if(!ctx) {
        print_colored("Impossible de créer le contexte SSL\n", COLOR_RED);
    ...
    printf("Handshake SSL...\n");
    if(SSL_connect(ssl) <= 0) {
        print_colored("Erreur handshake SSL\n", COLOR_RED);
        ERR_print_errors_fp(stderr);
        SSL_free(ssl);
        close(sockfd);
        SSL_CTX_free(ctx);
    ...
    print_colored("Connexion SSL établie\n", COLOR_GREEN);

    int len = snprintf(buffer, BUFFER_SIZE,
        "%s %s HTTP/1.1\r\n"
        "Host: %s\r\n"
        "User-Agent: HTTPClient-C/1.0\r\n"
        "Accept: */*\r\n",
        get_method_name(request->method),
        request->path,
        request->hostname);
    ...
    if(SSL_write(ssl, buffer, len) <= 0) {
        print_colored("Erreur envoi SSL\n", COLOR_RED);
        ERR_print_errors_fp(stderr);
        SSL_free(ssl);
        close(sockfd);
        SSL_CTX_free(ctx);
    ...
    printf("\nRéception de la réponse...\n");
    while((bytes_received = SSL_read(ssl, buffer, BUFFER_SIZE - 1)) > 0 && total_bytes < response_size - 1) {
        buffer[bytes_received] = '\0';
        strncat(response, buffer, response_size - total_bytes - 1);
        total_bytes += bytes_received;
    }
